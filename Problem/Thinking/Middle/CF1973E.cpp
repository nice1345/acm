...
设mi为不在位置上的最小数,ma为不在位置上的最大数
 mi<=i<=ma
 条件1：l<=mi+len//不然mi只能与大于n的值交换，这不可能
 条件2：r>=ma+1//不然ma只能与小于1的值交换，这不可能
 条件3：又l<r  r>=max(l+1,ma+1)
 mi+1 ma+1 mi+len ma+len
       r    l
 结合条件123  [l,r]一定包含[ma+1,mi+len]的1个数m使得 每个i mi<=i<=ma都有位置可换
 这样确保每个i mi<=i<=ma都有位置可换

 m=c+i

 对于保每个i [l,r]存在c l<=c+i<=r,l<r c与i可以交换
 将m取到max[ma+1,mi+len]&&[l,r]([l,r]一定包含[ma+1,mi+len]的1个数m,所以不为空) 又l<r
 l<=c+i-1<=r,l<r c与i-1可以交换
如果l==min+len [ma+1,mi+len]&&[l,r]==mi+len
 将m取到min[ma+1,mi+len]&&[l,r]==min+len([l,r]一定包含[ma+1,mi+len]的1个数m,所以不为空) 又l<r
i mi  ma
c len   mi+len-ma
 l<=c+i+1<=r,l<r c与i+1可以交换
